...
This report documents the analysis and improvement of a ChatGPT-generated traffic simulation built on a real road network loaded from OpenStreetMap. The original notebook contained several bugs that prevented it from running, and its congestion model (a binary threshold that either allows or blocks all movement) fails to capture how real traffic behaves.
...
Both are cellular automata, but at different scales. NaSch is \textit{microscopic}: it tracks individual car decisions (accelerate, brake, hesitate). Our model is \textit{macroscopic}: it tracks aggregate density and derives speed from it. The microscopic approach captures phenomena like phantom traffic jams (spontaneous stop-and-go waves), while the macroscopic approach captures how network topology distributes and concentrates congestion. The bonus section exploits this CA-on-a-network framing to study how different graph topologies affect the onset of gridlock.
...
The improved model addresses the structural problems of the original, but it introduces its own simplifications. These are deliberate trade-offs: each one makes the simulation more tractable while preserving the qualitative behavior we care about (relative congestion patterns and topology effects).

\paragraph{Linear speed-density relationship.}
The Greenshields model assumes that speed decreases \textit{linearly} with density. In reality, the speed-density relationship is nonlinear; alternative models such as Greenberg (logarithmic) and Underwood (exponential) better capture the sharp speed drop near jam density. We chose Greenshields because it has a single parameter ($\rho_{\text{jam}}$), is analytically transparent, and produces qualitatively correct behavior: free flow at low density, gradual slowdown, and gridlock at jam density. Since our analysis focuses on \textit{relative} congestion patterns (which edges congest more than others) rather than precise speed values, the linear approximation is sufficient.
...
The original visualizations (Section~3) show where cars \textit{went}. These new visualizations show where cars \textit{slowed down}. Same network, same routing, but the improved model reveals congestion dynamics that the binary threshold cannot capture.
...
\textbf{Edge betweenness centrality} is the rare exception: a metric defined directly on edges. It measures the fraction of all shortest paths in the network that traverse a given edge. Since our cars route via shortest paths, this is a direct structural prediction of traffic demand. Edges on many shortest paths should attract more cars and therefore experience higher density.

\subsection{Correlation Analysis}

We compute edge betweenness centrality (weighted by \texttt{travel\_time} to match how cars route) and correlate it against the empirical density from the multi-run analysis using the Spearman rank correlation coefficient \citep{spearman1904}. Unlike Pearson's correlation, which measures the strength of a \textit{linear} relationship between raw values, Spearman's correlation first converts both variables to ranks and then measures whether higher-ranked betweenness edges consistently correspond to higher-ranked density edges. This rank-based approach is appropriate here for two reasons: (1)~the relationship between betweenness and density is monotonic but not linear (the scatter plot shows increasing variance at higher betweenness values), and (2)~we care about the \textit{ordering} question (``can betweenness predict \textit{which} edges congest?'') rather than the exact functional form. Pearson correlation yields $r = 0.50$, underestimating the association by penalizing the nonlinearity, while Spearman yields $r = 0.88$, correctly reflecting the strong rank-order agreement.
...
The fact that edge betweenness centrality predicts congestion on a network it was not calibrated on supports a general conclusion: in any road network where vehicles follow shortest paths, edges that lie on many shortest paths will attract disproportionate traffic, leading to higher density. This is a structural property of shortest-path routing, not an artifact of a particular network layout. The slight decrease in correlation from Corrientes to Palermo suggests that network regularity (grid structure) strengthens the prediction, but even irregular networks exhibit the relationship clearly.
...
The three curves in Figure~\ref{fig:speed_vs_cars} tell qualitatively different stories about how each topology handles increasing load:
...
The tight standard error of the mean bands confirm that these profiles are stable properties of each topology, not artifacts of randomness.
...
This result illustrates a general principle: the same local rule can produce qualitatively different global behavior depending on the structure of the system. The Greenshields update rule is identical across all three topologies, yet the grid produces a plateau, the star produces a cliff, and the BA network produces a smooth slope.
...
This project was completed with the assistance of Claude (Anthropic), used to format and structure the code and written text. All analysis decisions, experiment design, and interpretation of results are my own.
